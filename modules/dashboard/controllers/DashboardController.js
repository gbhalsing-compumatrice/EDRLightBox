angular.module('EDRLightbox').controller('DashboardController', ['$scope', '$state', '$location', '$document', '$timeout', '$http', '$filter', 'NgTableParams', 'DashboardService', '$modal', function ($scope, $state, $location, $document, $timeout, $http, $filter, NgTableParams, DashboardService, $modal) {
    ...
    $scope.selectedRow = "";
    $scope.address = {};
    var pastRowId, pastSelectedTabId, pastleftSelectedTabId,additionaldata,modalInstance;
    $scope.additionalInfo = [];
    $scope.selectAll = true;
    $scope.selectAllModified = false;
    $scope.isRowDisabled = false;
    $scope.allItemChecked = false;
    $scope.selectAllAddr = true;
    $scope.sitesData = [];
    bindSitesData();
    ...
    function bindSitesData() {
        DashboardService.getSitesData().then(function (result) {
            ...
            for (i = 0; i < $scope.sitesData.length; i++) {
                $scope.sitesData[i].rowclass = "tableicongray fa fa-circle";
            }
                       
            $scope.findingRecordsCount = result.length;
            bindDBNameList();
            bindAddressList();
            bindColumnFilterList();
        }, function (error) {
    ...
    function bindColumnFilterList() {
        DashboardService.getColumnFilterList().then(function (result) {
          
            $scope.checkboxModel = result;
            //$scope.filterItems = result.db;
            //console.log("dbxxxxxxxxxx " + JSON.stringify(result.db))
        }, function (error) {
    ...
    function bindAddressList() {
        DashboardService.getAddressList().then(function (result) {
    ...
    $scope.getleftSelectedTab = function (value) {
        setleftSelectedTab(value);
        $scope.leftmenubtnValue = value

    }

    function setleftSelectedTab (value) {
    ...
    function setSelectedTab(value) {
        ...
        if (pastSelectedTabId == value && pastSelectedTabId != undefined) {
            ...
            checkClass = $("#panel").hasClass("panelwidthoff");
            if (checkClass) {
    ...
    $scope.getSelectedRowData = function (siteid) {
        ...
        var checkclick = $("#" + siteid).hasClass('tableicon fa-ban');
    ...
    function bindSitePopupData(siteid) {
        ...
        } else if ($scope.btnValue == 4) {
            $scope.siteDatabase = data.date;
            $scope.siteDirection = data.use;
            $scope.siteElevation = data.type;
        }
    ...
    function findById(siteid) {
        ...
        if ($scope.btnValue == 2) {
            localStorage.setItem("dataFrom", "findings");
            $scope.isphysicalTabOn = false;

            checkClass = $("#divAddInfo").hasClass("popoutscroll");

            if (checkClass) {
                $('#divAddInfo').removeClass('popoutscroll');
                $('#divAddInfo').addClass('popoutscrollFindings');
            }
        ...
        {
            localStorage.setItem("dataFrom", "physical");
            $scope.isphysicalTabOn = true;

            checkClass = $("#divAddInfo").hasClass("popoutscrollFindings");

            if (checkClass) {
                $('#divAddInfo').removeClass('popoutscrollFindings');
                $('#divAddInfo').addClass('popoutscroll');
            }
            ...
            $scope.isphysicalTabOn = false;
            localStorage.setItem("dataFrom", "other");
            for (var i = 0; i < $scope.buildingPermitData.length; i++) {
                if ($scope.buildingPermitData[i].Id === siteid) {
                    return $scope.buildingPermitData[i];
                }
            }
            throw "Couldn't find site with id: " + siteid;
        }
    ...
    // DB Filter 
    $scope.filterItems = {
        'RCRA-LQG': true, 'RCRA-SQG': true, 'RCRA-CESQG': true, 'LUST': true, 'SLIC': true, 'UST': true, 'CA FID UST': true,
        'HIST UST': true, 'SWEEPS UST': true, 'EDR US Hist Cleaners': true, 'EDR US Hist Auto Stat': true, 'CUPA Listings': true,
        'DRYCLEANERS': true, 'FINDS': true, 'HIST UST': true, 'Include Orphans in Site List': true
    };
    ...
    $scope.getDBFilterData = function (item, index) {
        var checkVal = $('#dbListId' + index).is(":checked");
        console.log("checkbox value " + checkVal);
        var count = 0;
        for (var keys in $scope.filterItems) {

            if (keys == item.database) {
                if (checkVal) {
                   
                    $scope.filterItems[keys] = true;
                   
                }
                else {
                    $scope.filterItems[keys] = false;
        ...
        for (var keys in $scope.filterItems) {
            ...
            var x = $('#dbListId' + count).is(":checked");
            if (!x) {
    ...
    $scope.selectAllDatabases = function () {
        console.log("reached" + $scope.selectAll);
        if ($scope.selectAll == true) {
    ...
    // class Filter
    $scope.setRowFilter = function (id,classname) {

        var checkClass = classname
        if (checkClass == "tableicongray fa fa-circle") {
           
            var classGreen = "tableicongreen fa fa-check-circle";
            applyclass(classGreen, id);
            $scope.selectedRowBanId = "";

        }
        else if (checkClass == "tableicongreen fa fa-check-circle") {
            
            var classGreen = "tableicon fa fa-ban";
            applyclass(classGreen, id);
            
            $scope.selectedRowBanId = id;
        }
        else if (checkClass == "tableicon fa fa-ban") {
           
            var classGreen = "tableicongray fa fa-circle";
            applyclass(classGreen, id);
            $scope.selectedRowBanId = "";
    ...
    function applyclass(classname, id) {

        for (i = 0; i < $scope.sitesData.length; i++) {
            if ($scope.sitesData[i].Id === id) {
                $scope.sitesData[i].rowclass = classname
             }
        }
    }

    $scope.findingsTDClass = [
                      { classname: "tableicongray fa fa-circle" },
                      { classname: "tableicongreen fa fa-check-circle" },
                      { classname: "tableicon fa fa-ban" }
    ];
   
    $scope.filterClass = {
        'tableicongray fa fa-circle': true,
        'tableicongreen fa fa-check-circle': true,
        'tableicon fa fa-ban': true
    };

    $scope.CSFilter = function (sites) {
       
        return $scope.filterClass[sites.rowclass];
    };

    $scope.getClassFilterData = function (index) {
    ...
    $scope.address = {
        ...
        'W MISSION ST': true,
        'DE LA VINA ST': true,
        'PUEBLO AND BATH': true,
        'CHAPALA ST': true,
        'DE LA GUERTA ST': true,
        'STATE ST': true,
        'DOWNTOWN SANTA BARBARA': true
    };

    $scope.AddressFilter = function (sites) {

        if ($scope.selectAllAddr == true) {
            return true;
        }
        else {
           
            var tempAddress = sites.address.replace(/[0-9]/g, '').replace(' ', '');
            for (var keys in $scope.address) {
                
                console.log("addres here is now " + tempAddress + '  ' + keys);
                console.log(tempAddress.length + '  ' + keys.length);
                if (keys == tempAddress) {
                    console.log("in if");
                    console.log('returning ' + keys + ' ' + tempAddress);
                    return $scope.address[keys];
                }
            }
        }
    }

    $scope.getAddressFilterData = function (item, index) {

        var checkVal = $('#addrListId' + index).is(":checked");
        var count = 0;
       
        var tempAddress = item.address.replace(/[0-9]/g, '').replace(' ', '');
        for (var keys in $scope.address) {
            ...
            if (keys == tempAddress) {
                ...
                if (checkVal) {
                    ...
                    $scope.address[keys] = true;
                }
                else {
                    $scope.address[keys] = false;
                }
            }
        }
        for (var keys in $scope.address) {

            var x = $('#addrListId' + count).is(":checked");
            if (!x) {
                $scope.selectAllAddr = false;
                break;
            }
            else {
                $scope.selectAllAddr = true;
    ...
    $scope.selectAllAddress = function () {
        console.log('selectAllAddr in select all address ' + $scope.selectAllAddr);
        if ($scope.selectAllAddr == true) {

            for (keys in $scope.address) {
